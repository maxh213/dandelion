Feature: 005 - Codex panel that reflects its login mode
  ...
  A line starting "Logged in using an API key" means API-key mode. Otherwise a line containing "ChatGPT" means ChatGPT mode.
  ...
  Scenario Outline: Codex failures render a dim panel, keep the others, and reap the child
    ...
    Examples:
      ...
      | is not in the PATH                                                                          | codex CLI not found in PATH                         | within 5 seconds       |
      | prints "Not logged in" for `login status` and exits 1                                       | codex is not logged in                              | within 5 seconds       |
      | prints "hello" for `login status` and exits 0                                               | codex is not logged in                              | within 5 seconds       |
      | hangs on `login status`                                                                     | Command timed out after 15s                         | after 15 to 25 seconds |
      | is in ChatGPT mode and answers id 2 with {"error":{"code":-32600,"message":"chatgpt authentication required to read rate limits"},"id":2} | chatgpt authentication required to read rate limits | within 5 seconds |
      | is in ChatGPT mode and answers id 2 with {"error":{"code":-32600},"id":2}                     | codex app-server error                              | within 5 seconds       |
      | is in ChatGPT mode and answers initialize but never answers id 2                            | codex app-server did not answer within 30s          | after 30 to 40 seconds |
      | is in ChatGPT mode and its app-server exits at once without output                          | codex app-server exited without answering           | within 5 seconds       |
      | is in ChatGPT mode and answers id 2 with {"id":2,"result":{"rateLimits":{"primary":null,"secondary":null}}} | Could not parse rate limits from response | within 5 seconds    |
      | is in ChatGPT mode and answers id 2 with {"id":2,"result":null}                              | Could not parse rate limits from response           | within 5 seconds       |
  ...
  Scenario: No CLI on the PATH at all
    Given the PATH contains no `claude`, `agy`, `kimi`, `codex` or `kilo` and ALLOWANCE_GROK_HOME is an empty directory
    When the user runs `npm start`
    Then the process exits with code 0
    And six dim unavailable panels render in order:
      | name   | reason                                   | caption              |
  ...
  Scenario: Earlier end-to-end checks keep passing
    ...
    Then every e2e passes, including the 001 to 004 e2es and the new "qa/005-codex.e2e.mjs", whether or not the tester has a real codex
    And "qa/005-codex.e2e.mjs" creates its fixture dir with the temp prefix "allowance-qa-005-" and runs `npm start` twice with PATH set to only its fixture dir and node's dir, NO_COLOR set, and an outer spawn timeout
    ...
    And no e2e invokes a real `claude`, `agy`, `kimi`, `grok`, `codex` or `kilo` binary
