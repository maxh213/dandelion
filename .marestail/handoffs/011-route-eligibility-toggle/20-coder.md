# 011 route-eligibility-toggle — coder (gate config revert after 19-hardener)

## Done
- Hardener 1: ran `git checkout 6b86a93 -- .dependency-cruiser.cjs`. This removes the `live-session-gets-eligibility-injected`
  rule that 12-architect added, so the file matches its bytes from before the task and the task's diff has no gate config change.
- No source or test changed. `src/app/live.ts` still imports only `render/index.ts` and `probes/index.ts`; the Eligibility is
  injected by `app/index.ts`.
- `marestail gate --tier fast`: GATE PASSED (606 tests, 301 functions, dependency rules kept).

## Config change
- None in this task any more. If maintainers want the stricter rule (live.ts may import only render/index.ts and probes/index.ts),
  they can add it outside this task; the current code already satisfies it.
