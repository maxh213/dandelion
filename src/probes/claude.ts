...
function parseWindowLine(line: string, now: string): UsageWindow | undefined {
  const match = WINDOW_LINE.exec(line.trim());
  if (!match) return undefined;
  const resetsAt = match[4] === undefined ? undefined : parseReset(match[4], now);
  return windowOf(windowLabel(match[2]), Number(match[3]), resetsAt);
}

function parseClaudeUsage(stdout: string, now: string): UsageWindow[] {
  const windows = presentOnly(stdout.split('\n').map((line) => parseWindowLine(line, now)));
  return windows.some((window) => window.label === 'weekly') ? windows : [];
}

export const claudeProbe: WindowProbe = {
  id: 'claude',
